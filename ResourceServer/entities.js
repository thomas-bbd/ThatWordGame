...
export class Game {
  ...
  start = (player_id, websocket) => {
    if (
      player_id === this.game_master.player_id &&
      websocket === this.game_master.websocket
    ) {
  ...
  next = () => {
    ...
    while (this.game_lineup[this.turn].is_eliminated())
      this.turn = (this.turn + 1) % this.game_lineup.length;
  ...
  play = (word, player_id, websocket) => {
    if (
      player_id === this.curr_player.player_id &&
      this.curr_player.websocket === websocket
    ) {
